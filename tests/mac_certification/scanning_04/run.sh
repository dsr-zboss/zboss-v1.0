...
./zdo_start_zc_s04 /tmp/st0.write /tmp/st0.read &
coordPID=$!
...
echo "run ze"
./start_ze_s04 /tmp/st1.write /tmp/st1.read &
endPID=$!
wait_for_start ze
...
if grep -A 2 "beacon payload dump" tst_ze*.log
then
  if grep "pan desc:" tst_ze*.log
  then
    echo "DONE. Check Pan Descriptor and beacon payload"
  else
    echo "ERROR. TEST FAILED, pan descriptor not found!!!"
  fi
else
  echo "ERROR. TEST FAILED, beacon payload not found!!!"
fi
