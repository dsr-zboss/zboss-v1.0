...
wait_for_start() {
    ...
    do
        sleep 1
        s=`grep Device tp_aps_bv_33_i_${nm}*.log`
    done
    if echo $s | grep OK
    then
        return
...
./tp_aps_bv_33_i_DUTZC ${PIPE_NAME}0.write ${PIPE_NAME}0.read &
coordPID=$!
...
echo "run zed1"
./tp_aps_bv_33_i_gZED1 ${PIPE_NAME}1.write ${PIPE_NAME}1.read &
end1PID=$!
...
echo "run zed2"
./tp_aps_bv_33_i_gZED2 ${PIPE_NAME}2.write ${PIPE_NAME}2.read &
end2PID=$!
...
if grep "Test is finished, status OK" tp_aps_bv_33_i_gZED1*.log
then
