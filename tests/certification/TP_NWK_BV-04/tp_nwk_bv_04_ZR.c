...
#ifndef ZB8051
  ZB_INIT("tp_nwk_bv_04_ZR", argv[1], argv[2]);
#else
  ZB_INIT("tp_nwk_bv_04_ZR", "1", "1");
#endif
...
void zb_zdo_startup_complete(zb_uint8_t param) ZB_CALLBACK
{
  ...
  if (buf->u.hdr.status == 0)
  {
    ...
    {
      zb_leave_req(param);
      ZB_SCHEDULE_ALARM(send_data, param, ZB_MILLISECONDS_TO_BEACON_INTERVAL(10000));
      ZB_SCHEDULE_ALARM(zb_start_join, param, ZB_MILLISECONDS_TO_BEACON_INTERVAL(15000));

      /* rejoin after leave from ZC - we don't need it, just need an error from buffertest*/
      /*ZB_SCHEDULE_ALARM(zb_start_join, param, ZB_MILLISECONDS_TO_BEACON_INTERVAL(80000));*/
      ZB_SCHEDULE_ALARM(send_data, param, ZB_MILLISECONDS_TO_BEACON_INTERVAL(40000));
      flag = ZB_FALSE;
    }
  }
  else
