...
static zb_uint_t test_counter = 0;
...
void simple_desc_callback(zb_uint8_t param) ZB_CALLBACK
{
  ...
  resp = ZB_GET_SIMPLE_DESC_RESP(zdp_cmd);
...
void get_node_desc(zb_uint8_t param) ZB_CALLBACK
{
  zb_buf_t *asdu;
...
void zb_zdo_startup_complete(zb_uint8_t param) ZB_CALLBACK
{
  ...
  if (!test_counter++)
  {
    if (buf->u.hdr.status == 0)
    {
      ...
      /* wait a little to get acks for device announce */
      ZB_SCHEDULE_ALARM(get_node_desc, 0, ZB_TIME_ONE_SECOND*3);
    }
    else
    {
