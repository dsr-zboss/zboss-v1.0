...
static void ieee_addr_req(zb_buf_t *buf, zb_apsde_data_indication_t *ind);
...
MAIN()
{
...
#ifndef ZB8051
  ZB_INIT("zdo_test_nwk_addr_zr", argv[1], argv[2]);
#else
  ZB_INIT("zdo_test_nwk_addr_zr", "2", "2");
#endif
#ifdef ZB_SECURITY
   ...
   * been connected to it */
  {
    ...
    ZB_IEEE_ADDR_COPY(ZB_PIB_EXTENDED_ADDRESS(), &ieee_address);
  }
...
void ieee_addr_callback(zb_uint8_t param) ZB_CALLBACK
{
  ...
  {
    TRACE_MSG(TRACE_APS2, "IEEE_addr :  received Ok", (FMT__0));
  }
  else
  {
    TRACE_MSG(TRACE_ERROR, "IEEE_addr : ERROR receiving %x", (FMT__D, status));
  }
}
